...
int main(int argc, char *argv[]) {
  ...
  while((opt = getopt(argc, argv, "i:h:p:")) >=0 ){
      switch (opt) {
          case 'i':
              iface_name = optarg;
              break;
          case 'h':
              cfg->setServerHost(optarg);
              break;
          case 'p':
              cfg->setServerPort(stoi(optarg));
              break;
          default:
              cout << "parameters ERR"<<endl;
              break;
      }
  }
  ...
  if (pcap_hdl == nullptr) {
    cout << "Could not open PCAP on interface" << iface_name << errbuf << endl;
    return -1;
  }
